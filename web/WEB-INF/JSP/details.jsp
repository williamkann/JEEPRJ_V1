...
<html>
    ...
    <body>
        ...
        out.println("<input type=\"hidden\" value=\"" + employee.getId() + "\" name=\"id\"/>");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"name\">Nom</label>");
        out.println("<input type=\"text\" name=\"name\" value = \"" + employee.getName() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"first_name\">Prénom</label>");
        out.println("<input type=\"text\" name=\"first_name\" value = \"" + employee.getFirst_name() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"homePhone\">Tél dom</label>");
        out.println("<input type=\"text\" name=\"homePhone\" value = \"" + employee.getHome_phone() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"mobilePhone\">Tél mob</label>");
        out.println("<input type=\"text\" name=\"mobilePhone\" value = \"" + employee.getMobile_phone() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"workPhone\">Tél pro</label>");
        out.println("<input type=\"text\" name=\"workPhone\" value = \"" + employee.getWork_phone() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"address\">Adresse</label>");
        out.println("<input type=\"text\" name=\"address\" value = \"" + employee.getAddress() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"postalCode\">Code Postal</label>");    
        out.println("<input type=\"text\" name=\"postalCode\" value = \"" + employee.getPostal_code() + "\" size=\"35\">"); 
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"city\">Ville</label>");    
        out.println("<input type=\"text\" name=\"city\" value = \"" + employee.getCity() + "\" size=\"35\">");
        out.println("</div>");
        out.println("<div>");
        out.println("<label for=\"city\">Email</label>");    
        out.println("<input type=\"text\" name=\"email\" value = \"" + employee.getEmail() + "\" size=\"35\">");
        out.println("</div>");
        if(ERR_EMAIL != null)
        ...
        out.println("<input type=\"submit\" class=\"btn btn-success\" name=\"detailAction\" value=\"Save\"/>");
